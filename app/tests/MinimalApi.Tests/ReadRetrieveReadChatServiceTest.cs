...
public class ReadRetrieveReadChatServiceTest
{
    ...
    public async Task NorthwindHealthQuestionTest_TextOnlyAsync()
    {
        ...
        documentSearchService.QueryDocumentsAsync(Arg.Any<string?>(), Arg.Any<float[]?>(), Arg.Any<RequestOverrides?>(), Arg.Any<CancellationToken>())
                ...
                {
                    new SupportingContentRecord("Northwind_Health_Plus_Benefits_Details-52.pdf", "https://st65r2fo2xbeufi.blob.core.windows.net/content/", "The Northwind Health Plus plan covers a wide range of services related to the treatment of SUD. These services include inpatient and outpatient treatment, counseling, and medications to help with recovery. It also covers mental health services and support for family members of those with SUD"),
                    new SupportingContentRecord("Northwind_Health_Plus_Benefits_Details-90.pdf", "https://st65r2fo2xbeufi.blob.core.windows.net/content/", "This contract includes the plan documents that you receive from Northwind Health, the Northwind Health Plus plan summary, and any additional contracts or documents that you may have received from Northwind Health. It is important to remember that any changes made to this plan must be in writing and signed by both you and Northwind Health."),
                });
